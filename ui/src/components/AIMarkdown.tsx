...
const components: Components = {
  ...
  h1: ({ children }) => (
    <p className="mt-2 mb-1 text-sm font-semibold text-slate-800 dark:text-slate-100">
      {children}
    </p>
  ),
  h2: ({ children }) => (
  ...
  h3: ({ children }) => (
    <p className="mt-2 mb-1 text-sm font-semibold text-slate-700 dark:text-slate-200">
      {children}
    </p>
  ),
  h4: ({ children }) => (
  ...
  p: ({ children }) => (
    <p className="mb-2 last:mb-0 text-sm leading-relaxed text-slate-700 dark:text-slate-200">
      {children}
    </p>
  ),
  ul: ({ children }) => (
    <ul className="mb-2 last:mb-0 list-disc pl-5 space-y-1 marker:text-slate-400 dark:marker:text-slate-500 text-sm text-slate-700 dark:text-slate-200">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="mb-2 last:mb-0 list-decimal pl-5 space-y-1 marker:text-slate-400 dark:marker:text-slate-500 text-sm text-slate-700 dark:text-slate-200">
      {children}
    </ol>
  ),
  ...
  blockquote: ({ children }) => (
    <blockquote className="mb-2 last:mb-0 border-l-2 border-slate-300 dark:border-surface-600 pl-3 italic text-slate-600 dark:text-slate-300">
      {children}
    </blockquote>
  ),
  code: ({ className, children, ...rest }) => {
    ...
    // ``pre`` slot below renders the block frame, this slot styles the
    // text.
    ...
    return (
      <code
        className="rounded bg-slate-100 dark:bg-surface-700 px-1 py-0.5 font-mono text-xs text-slate-800 dark:text-slate-100"
        {...rest}
      >
        {children}
      </code>
  ...
  pre: ({ children }) => (
    <pre className="mb-2 last:mb-0 overflow-x-auto rounded bg-slate-100 dark:bg-surface-900 p-2 text-xs text-slate-800 dark:text-slate-100">
      {children}
    </pre>
  ),
  table: ({ children }) => (
    <div className="mb-2 last:mb-0 overflow-x-auto">
      <table className="min-w-full text-xs text-slate-700 dark:text-slate-200 [font-variant-numeric:tabular-nums]">
        {children}
      </table>
    </div>
  ),
  ...
  // Drop images entirely — the model has no legitimate reason to embed
  // an external image in a risk explainer, and admitting them is an
  // injection / data-exfil surface (remote URLs ping a logger).
  img: () => null,
}
