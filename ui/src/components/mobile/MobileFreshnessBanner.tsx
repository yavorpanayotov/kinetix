...
import type { FreshnessLevel } from '../../utils/freshnessLevel'
...
// lands on the big number first, so the staleness warning must read as heavier
// than the data it sits above — larger padding and type.
const SIZE_CLASSES: Record<FreshnessLevel, string> = {
  fresh: 'px-4 py-2 text-sm',
  amber: 'px-4 py-2 text-sm',
  red: 'px-4 py-3 text-base',
}
...
export function MobileFreshnessBanner({ dataAsOf }: MobileFreshnessBannerProps) {
  ...
  return (
    <div
      ...
      role="status"
      aria-live="polite"
      className={`w-full font-medium ${SIZE_CLASSES[level]} ${BANNER_CLASSES[level]}`}
    >
