...
/**
 * Mock WebSocket that opens successfully and stays open. Used by tests that
 * need to verify maintenance-banner behaviour without the page proxy
 * connecting to a real backend and dropping into the reconnecting state
 * (which would outrank maintenance in the SystemStatusBanner priority order).
 */
async function injectStableOpenWebSocket(page: import('@playwright/test').Page) {
  await page.addInitScript(() => {
    ...
    ;(window as unknown as Record<string, unknown>).WebSocket = function (
      ...
      if (urlStr.includes('/ws/prices')) {
        return new OpenWebSocket(url, protocols)
      }
...
test.describe('System status banner consolidation (§4.1)', () => {
  ...
  test('maintenance banner is shown and no other system banner is stacked under DEGRADED health', async ({
    ...
    // Stabilise the WebSocket so the page does not flicker into the
    // reconnecting state (which outranks maintenance in the SystemStatusBanner
    // priority order) just because the dev-server WS proxy cannot reach a
    // real backend in test.
    await injectStableOpenWebSocket(page)
  ...
  test('only one role=alert | role=status live region from the system bar is in the DOM at a time', async ({
    ...
    // Same rationale as the first test in this describe — stabilise the WS so
    // maintenance does not get superseded by a transient reconnecting banner.
    await injectStableOpenWebSocket(page)
