...
- **[ADR Index](ADR-Index)** — all 37 architecture decisions with summaries and triggers
...
| **Risk engine** | Python 3.12 — NumPy, SciPy, PyTorch — 9 gRPC services |
...
| **Behavioural specs** | 25 Allium v3 specifications |
| **Architecture decisions** | 37 ADRs |
| **Tests** | 11,300+ across Kotlin (Kotest), Python (pytest), UI (Vitest + Playwright) |
...
5. **Specs are contracts.** 25 Allium specifications encode behavioural invariants. Code that diverges from a spec is a bug — in code or in the spec.
