...
- [ ] **4.4** Pending-new correlator (`PendingNewCorrelator.kt`): maps `clOrdID` → `CompletableDeferred<PlaceOrderResponse>` so the inbound 35=8 handler can wake the synchronous RPC.
