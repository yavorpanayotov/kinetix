...
3. ⚠ **VaR-vs-pricing Greeks confusion in intraday P&L.** `risk-orchestrator/.../IntradayPnlService.kt:190-195` reads `delta/gamma/vega/theta/rho` from `DailyRiskSnapshot` (VaR sensitivities). Spec explicitly warns "Use PRICING sensitivities, NOT VaR sensitivities from greeks.py" (`intraday-pnl.allium:225-231`). Attribution numbers are arithmetically wrong. **Quant verdict (2026-05-01):** build `SodGreekSnapshot` infrastructure (Path A) — VaR Greeks are not a degraded approximation of pricing Greeks but a different quantity entirely (`d(VaR)/dS` vs `dV/dS`); calling them a proxy in attribution will systematically inflate `unexplained_pnl` and train operators to ignore real `dataQualityWarning` fires. Implementation is bounded but sprint-sized (new entity + repository + Flyway migration + gRPC pricing-Greek method + SOD population job + IntradayPnlService rewire) and is queued out of the cleanup pass. Linked to P2 #30 (frozen SOD cache) and ADR-0032 placeholder for the intraday-Greek source decision.
