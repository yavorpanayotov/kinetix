...
3. ✓ **VaR-vs-pricing Greeks confusion in intraday P&L.** Spec at `intraday-pnl.allium:225-231` mandates pricing sensitivities, not VaR sensitivities from `greeks.py`. **Quant verdict (2026-05-01):** Path A — build `SodGreekSnapshot` infrastructure. **Phase 1 (2026-05-01):** persistence layer was already in place; `IntradayPnlService` now reads from `SodGreekSnapshotRepository` via the same VaR-Greek fallback pattern as `PnlComputationService`, with five new test scenarios. **Phase 2 (2026-05-01):** end-to-end population path now wired: new `CalculatePricingGreeks` gRPC RPC on `risk-engine` (Black-Scholes closed-form for options, DV01 for FI, identity delta for linear instruments — wraps existing `bs_greeks`/`bond_dv01` modules) + Kotlin `PricingGreeksClient` + `SodSnapshotService.persistPricingGreeks` hook that runs alongside the VaR snapshot. SOD job is best-effort: pricing-Greek failures are logged and swallowed so consumers fall back to VaR Greeks per-instrument. Eight Python integration tests cover each asset-class dispatch + per-instrument failure isolation; three Kotlin tests cover the SOD-job hook (wired-and-persisted, null-client legacy, client-throws fallback). Per ADR-0029 the engine remains a pure calculator and the orchestrator owns market data fetching. ADR-0032 is now implemented end-to-end and can be marked Accepted.
...
13. ✓ **`ExpireDayOrder` rule has no implementation.** Resolved (2026-05-01) per ADR-0035 (fix-gateway service extraction). State-side: `TimeInForce` enum (DAY/GTC/IOC/FOK/GTD) + `expires_at` column on `execution_orders` (V22 migration, GTC backfill for existing orders, DAY default for new orders). Five launch venues wired in `VenueCutoffRegistry` (NYSE, NASDAQ, LSE, TSE, HKEX) with timezone-aware cutoffs and weekend-closed handling. `ScheduledOrderExpirySweeper` runs every 60s, transitions DAY orders past venue cutoff and GTD orders past `expires_at` to `EXPIRED` via the existing `updateStatus` path. GTD validation: future + within max-GTD horizon. Cancel emission to the venue side is currently a logging stub (`LoggingOrderCancelEmitter`) — fix-gateway phases 3-4 (extracting the inbound `FIXExecutionReportProcessor` and outbound `PlaceOrder`) follow on a separate track. Tests: 7 GTD/TIF scenarios in `OrderSubmissionServiceTest`, 11 cutoff scenarios in `VenueCutoffRegistryTest`, 7 expiry scenarios in `ScheduledOrderExpirySweeperTest`.
