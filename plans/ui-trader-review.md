...
- [ ] **Fix asset-class taxonomy for Treasuries.** Find the classifier that maps `UST-5Y/UST-10Y/PG` to `EQUITY/Cash Equity`. Reference-data or instrument-master miscoding is the likely culprit. Failing test: `ReferenceDataAcceptanceTest` asserting `UST-10Y.assetClass == GOVERNMENT_BOND` and `UST-10Y.instrumentType != CASH_EQUITY`. Then fix the upstream seed/classifier and re-derive everywhere it propagates.
  ...
  Blocked: 2026-05-28 — verification failed for `./gradlew :reference-data-service:acceptanceTest --tests "*InstrumentTaxonomy*"` on independent re-runs (forced re-run, single-worker, daemon-restart):
  ```
  io.kotest.engine.spec.SpecInstantiationException: Could not create instance of class com.kinetix.referencedata.seed.InstrumentTaxonomyAcceptanceTest
  ...
  ```
  Diagnosis: the fix itself is correct — `./gradlew :position-service:acceptanceTest --tests "*PositionAssetClassAcceptanceTest"` (the second half of the acceptance pair) is GREEN, and `./gradlew :demo-orchestrator:test` is GREEN. The reference-data acceptance suite has a pre-existing infrastructure flake the prior subagent (`aa67323fe28f937ff`) called out in its follow-ups: each acceptance spec instantiates a HikariPool of size 8 against the Testcontainers Postgres whose `max_connections` is capped (see `TestcontainerCaps`), and pools are never closed across spec instances. Even with a `*InstrumentTaxonomy*` filter narrowing to the single new spec class, Kotest re-instantiates the spec while the previous pool is still draining and the second pool initialization hits the cap. The new `InstrumentTaxonomyAcceptanceTest` is the canary that exposes the latent issue; this commit ships the correct production fix but the verification command from the plan does not pass on a clean run. Unblock options: (a) reduce `DatabaseTestSetup.startAndMigrate()` HikariPool size to 2–3 and add an `AfterSpec` teardown that closes the pool; (b) share a single connection pool across acceptance specs in the module; (c) raise `max_connections` on the Testcontainers Postgres via container env vars. Any of these is a separate, infrastructure-only change and outside this checkbox's scope.
