...
/**
 * Acceptance test for the gateway's `POST /api/v1/insights/explain/var`
 * proxy. The gateway is a thin pass-through to `ai-insights-service`: it
 * must forward the request body verbatim and return the upstream response
 * body verbatim. We stand up a real embedded Netty server playing the role
 * of `ai-insights-service` on a random port so the proxy travels over real
 * HTTP — content negotiation, serialisation, and HTTP wire behaviour are
 * all exercised (per CLAUDE.md "Project Conventions": acceptance tests use
 * real HTTP for HTTP-only fakes).
 */
